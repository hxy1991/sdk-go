...
func WithCacheRefreshInterval(cacheRefreshInterval time.Duration) Option {
	return optionFunc(func(appConfig *EnhancedAppConfig) error {
		if cacheRefreshInterval < 15*time.Second {
			log.Warn("cacheRefreshInterval is too short, it will be reset to 15s")
			cacheRefreshInterval = 15 * time.Second
		} else if cacheRefreshInterval > 24*time.Hour {
			log.Warn("cacheRefreshInterval is too long, it will be reset to 24h")
